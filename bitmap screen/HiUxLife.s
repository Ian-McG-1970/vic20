...
;At the end of the screen we need to calculate the bottom
;row. The row below is the top row of the screen, but that's
;been overwritten so we need to pre-save the topRow.
...
main
	jsr startup ; setup screen and clear it.
...
main12
	ldy #0 ;row 0
main15
	ldx #8 ;pixels per byte.
	
main20
	jsr Rnd ;get next rnd.
	lda gSeed+1
...
;In LifeCtr we maintain a running count of the number
;of live cells. Assume the count is currently in neib,
;then as we go to the next bit, then set bits on the
;left would decrement the count and set bits that
;become the right most bits increment the count.
;But what happens at the beginning of a row? The count needs to
;be set so that the next shift calculates it correctly.
;Also what do we do about the middle bit? Perhaps the easiest
;way is to modify the output so that if the middle bit
;is set, then the count should be decremented.
...
LifeNuRo10
	...
	;now to prime cacheLefts, cache centre
	;and cache bot for this row. But the initial *L
	;cache bytes are in fact the bytes at the end of
	;the top, current and screen rows and the L
	;bytes are the current ones, but we take them from
	;the duplicate ones at the end, because then we
	;can index both.
	ldx #1	;src
	ldy #3	;dst
LifeNuRo20
	lda prevRow+kCols-1,x ;load prevRow+kCols first (i.e. prevRow).
	...
	cmp #kRows-1
	beq LifeCtr
	;now to prime cacheBottoms. (ptr1) points to the new
	;current row, so the row below is at y=1, unless you're
	;on the bottom row, where we need the top row instead.
	iny	;so y=1 now.
...
LifeCtr02
	...
	jmp LifeCtr06
LifeCtr03
	ldy #kBmH+1 ;normally br is next row down and next col
	lda (ptr1),y
LifeCtr04
	sta cacheBR
...
UpdCell75
	;so, we've added 160*kCols by now and need to
	;sub 160*kCols, then add 1.
	inc gRow
	sec
	lda ptr1
	sbc #<kBmNextRw
...
DoGen ;increment genStr, and display from col 4
...
DoGen30
	lda $8180,x ;start of char table for digits.
	sta (ptr1),y
...
;Setup screen and clear it.
startup ;82b long.
	lda #20 ;bit9 of addr=0, 20 columns, colorRam at $9400.
	sta 36866 ;A9:columns
	lda #128+20+1 ;frame1,2*rows,double-height.
	sta 36867 ;rows
...
startup10
	...
	sta 36869	;video ram and char set at $1000.
	rts
